...
const CONFIG = {
  ...
  function renderReviews() {
    DOM.reviewsSlider.innerHTML = STATE.reviews.map(review => reviewHTML(review)).join('');
    initReviewsSlider();
  }
  ...
  function initReviewsSlider() {
    if (typeof gsap !== 'undefined') {
      ...
      const slider = DOM.reviewsSlider;
  ...
  // Настройка эффекта курсора
  function setupCursorEffect() {
    if (DOM.cursor && DOM.cursorFollower) {
      ...
      const interactiveElements = document.querySelectorAll('a, button, .portfolio-item, .service-card, input, textarea, select');
      
      interactiveElements.forEach(el => {
        el.addEventListener('mouseenter', () => {
          DOM.cursor.style.transform = 'translate(-50%, -50%) scale(1.5)';
          DOM.cursorFollower.style.transform = 'translate(-50%, -50%) scale(1.5)';
          DOM.cursorFollower.style.borderColor = 'var(--primary)';
        });
  ...
  function setupEventListeners() {
    ...
    if (DOM.modalOverlay) {
      DOM.modalOverlay.addEventListener('click', () => toggleModal(DOM.modal, false));
    }
